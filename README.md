...
> 图片落盘、折叠条显示）。前端 175 个单测全绿，`tsc --noEmit` 无错，生产构建通过；
...
5. **虚拟滚动的视口位置只由锚点派生**——`offsetHeight` 量出来的行高、翻页插入的历史，都会让累计高度表 `offsets` 换代；坐标系一换，同一个 `scrollTop` 就指向别的内容了。所以每次换代都要先取锚点（**行 id + 行内偏移**）再反算 `scrollTop`，**不做任何增量加减**（见 `core/vscroll.ts` 的 `anchorKeyAt` / `topForKey`）。围绕它有一串红线，每一条单独都能让滚动抽风：
   - **视口只由「坐标系换代」驱动**（`planViewport`，有单测），所以那个 effect **刻意不读 `scrollTop`**——读了的话用户每滚一下它就重跑一遍"要不要校正视口"，补偿和用户的滚动互相打架（滚轮时灵时不灵、卡在中间）。用户的滚动只更新锚点，在 `onScroll` 里做。换会话/贴底追加才 `jump-bottom`，翻页那一轮返回 `hold` 让路（补偿由翻页流程自己负责），其余一律 `pin`（钉住锚点）。
   - **锚点存的是「行 key + 行内偏移」这个语义量本身**，不是 `(offsets 表, scrollTop)` 的组合。组合里的两个成员各有各的失效时机：effect 因别的依赖重跑时，`offsets` 已经换代而 `scrollTop` 还是旧的（或反过来），拿它取锚点就是"用新尺子量旧长度"，会取到**另一行**——实测视口停在 262px 之外的别的消息上。语义量与此无关，"用户在看哪一行的第几像素"换代前后说的是同一处内容。
   - **补偿要跑在微任务 / async 上下文里，不能放在 `createEffect` 里**（`scheduleRestore` / `restore`）。`createEffect` 执行期间 Solid 的更新队列是锁的：`setHeights` / `setScrollTop` 都不会立刻改 DOM，于是"测量 → 补偿 → 再测量"的迭代第二轮量到的还是同一批 DOM，等于没迭代。而迭代正是消除"锚点行前面那些从没渲染过、只有估值的行"的唯一手段——不迭代时翻页偏差是数百像素（实测 `scrollTop` 该走 3021px 却走了 3342px）。
   - **向上翻页的补偿由翻页流程自己做**（`paginateUp`）：必须在 `await loadOlder()` **之前**取好锚点（返回后再取，行序与高度表都已经换代），并在 `await` 之后立刻补——只有它同时握有"翻页前的锚点"和"写信号能同步推进 DOM 的上下文"。
