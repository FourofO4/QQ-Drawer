...
> 图片落盘、折叠条显示）。前端 183 个单测全绿，`tsc --noEmit` 无错，生产构建通过；
...
5. **虚拟滚动的视口位置以 DOM 真值为准，不以高度表为准**——`offsetHeight` 量出来的行高、翻页插入的历史，都会让累计高度表 `offsets` 换代；坐标系一换，同一个 `scrollTop` 就指向别的内容了。所以每次换代都要把「用户正看着**哪一行**、看到该行的**第几像素**」（锚点）保住。这里最关键的一条决定是：**锚点的位置从 DOM 现量，不从 `offsets` 反算**。高度表里混着未渲染行的**估值**（真机上短句 30px、图片行 184px，差一个数量级），按它反算出来的行号可以是错的——而锚点行一定在视口附近、一定在渲染窗口里，它的真实位置随时能量。围绕它有一串红线，每一条单独都能让滚动抽风：
   - **锚点的「行内偏移」允许为负**（`core/vscroll.ts` 的 `anchorFromTops`）。列表滚到最顶上时，第一行的顶边还在容器顶边**下方**（`.msgs` 有 12px 的 `padding-top`），也就是"视口顶边落在第一行上方那段空白里"。把这个偏移记成 0，补偿就会把这一行顶到容器顶边上 —— 翻一页差一个内边距，实测 12px，肉眼看得见。
   - **补偿是闭环：先用坐标系粗定位把锚点行拉进渲染窗口，再读它的 DOM 真值精确定位**（`settle` + `alignToAnchor`）。粗定位允许不准，因为精定位不查表 —— 任何一层估值出问题都传导不到最终位置。收敛判据是"位置没动、也没有新测量"，不是猜轮数。
   - **补偿只能有一条路径**。曾经翻页流程里也补了一次，它拿的是翻页**前**的锚点快照，和用户在这几百毫秒里滚出来的位置打架（"翻页时闪一下、落到别处"）。现在 `paginateUp` 只负责"别重复发请求"，位置统一由 `createEffect` 那一个入口处理。
   - **补偿跑在微任务里**（`scheduleRestore`），不能放在 `createEffect` 里直接做：那里 Solid 的更新队列是锁的，`setHeights` / `setScrollTop` 都不会立刻改 DOM，"量 → 修 → 再量"的闭环第二轮量到的还是同一批 DOM。
   - **前插一整批时先把渲染窗口从列表顶端铺满，把整批量准**（`setWide`）。只量渲染窗口盖得住的那部分，剩下的行的高度估值会在之后被逐个换成实测值，每换一次总高就变一次 —— 用户看到的就是"滚动条一直闪、长短不一"。
   - **视口只由「坐标系换代」驱动**（`planViewport`，有单测），所以那个 effect **刻意不读 `scrollTop`**——读了的话用户每滚一下它就重跑一遍"要不要校正视口"，补偿和用户的滚动互相打架（滚轮时灵时不灵、卡在中间）。用户的滚动只更新锚点，在 `onScroll` 里做。换会话/贴底追加才 `jump-bottom`，其余一律 `pin`（钉住锚点）。
   - **逻辑位置与真实位置同批更新**。`scrollTop` 信号（`range` 的唯一输入）只允许从 `writeTop` 一个口子写，写完顺手同步信号，并记**浏览器夹取后**的真实值——总高度刚变小时我们想写的位置可能越界，记想要的那个值等于让逻辑位置从此与实际错开，渲染窗口就会算出一段根本不在视口附近的行（"快速滚动会飞到很上面的聊天记录，过一会又滚回来"）。
   - **`overflow-anchor: none` 必须显式写上**，否则浏览器自带的 scroll anchoring 会在"上方内容变高"时自己改 `scrollTop`，与锚点补偿叠成双份偏移。
...
- **滚动、虚拟列表、布局这类"单测够不着"的行为，开 dev server 用真浏览器验**（`npm run dev`，然后 `http://localhost:5173/?latency=900`）。三条支撑：
  - `src/dev/mock.ts` 是完整的假后端（会话、历史、图片、窗口形态都齐），历史有 300 条且行高差异真实（短句 ~90px、长文 ~140px、图片行 ~200px），能把向上翻页整条路径跑起来；
  - `?latency=900` 把每页延迟拉到真机量级（真机要问本地库、必要时问 NapCat），否则"加载中用户还在滚"那一类问题根本不会出现；
  - `window.__qqDebug = { store, events }` 是开发期挂出来的把手，直接切会话/读状态/注入消息。**必须用它，不能自己 `import('/src/state/store.ts')`**——HMR 会让同一个模块出现两个实例（说明符被改写成带 `?t=` 的 URL），自己 import 拿到的是空状态。
  
  验证的判据也别看 `scrollTop`（插入内容时它会合法地大幅变化），要看**「视口顶端那条消息的 id」和它的行内偏移**——这两个量跨坐标系稳定，跳了才是真跳了。
- 界面本身走人工走查（规格 §5 的约定）。
